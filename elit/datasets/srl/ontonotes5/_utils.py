...
from elit.datasets.parsing.loaders._ctb_utils import remove_all_ec, convert_to_dependency
...
def convert_jsonlines_to_BIO(json_file, output_file=None, doc_level_offset=True, normalize_token=False):
    json_file = get_resource(json_file)
    if not output_file:
        output_file = os.path.splitext(json_file)[0] + '.ner.tsv'
    if normalize_token:
