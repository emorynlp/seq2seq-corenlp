...
from elit.utils.span_util import enumerate_spans
...
class CoNLL2012SRLDataset(TransformableDataset):
    ...
    def build_sample(self, sentence, deduplicated_srl, doc, sid):
        ...
        frame_ids = doc.get('frame_ids', None)
        if frame_ids is not None:
            sample['frame_ids'] = dict((int(k), v) for k, v in frame_ids[sid].items())
        return sample
...
def create_tags_for_wsd(sample: dict) -> dict:
    frame_dict = sample.get('frame_ids', None)
    if frame_dict is not None:
        frame_ids = [0] * len(sample['token'])
        for idx, frame in frame_dict.items():
            if not 0 <= idx < len(frame_ids):
                cprint(f'Invalid frame idx [red]{idx}[/red] found, 0 <= {idx} < {len(frame_ids)} expected.')
                continue
            _frame: str = frame[-2:]
            if _frame.isdigit():
                frame_ids[idx] = int(_frame)
            # else:
            #     flash(f'Invalid frame [red]{frame}[/red] found in sample {sample}')
        sample['wsd_id'] = frame_ids
        del sample['frame_ids']
    return sample
