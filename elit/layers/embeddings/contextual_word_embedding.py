...
from elit.layers.transformers.pt_imports import PreTrainedTokenizer, AutoConfig_, AutoTokenizer_
...
class ContextualWordEmbeddingModule(TransformerEncoder):
    ...
    def forward(self, batch: dict, mask=None, **kwargs):
        ...
        output: Union[torch.Tensor, List[torch.Tensor]] = super().forward(input_ids, token_span=token_span,
...
class ContextualWordEmbedding(Embedding, AutoConfigurable):
    def __init__(self, field: str,
                 ...
                 trainable:Union[bool, Optional[Tuple[int, int]]]=True) -> None:
        """A contextual word embedding builder which builds a
        :class:`~elit.layers.embeddings.contextual_word_embedding.ContextualWordEmbeddingModule` and a
        :class:`~elit.transform.transformer_tokenizer.TransformerSequenceTokenizer`.
