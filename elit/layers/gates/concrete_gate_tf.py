...
class ConcreteGateTF(tf.keras.layers.Layer):
    """
    ...
    def __init__(self, name, shape, temperature=0.33, stretch_limits=(-0.1, 1.1), l0_penalty=0.0, l2_penalty=0.0,
                 eps=1e-6, hard=False, local_rep=False, **kwargs):
        ...
        self.log_a = self.add_weight('log_a', shape)

    def call(self, values, training=None, axis=None):
        """ applies gate to values, if is_train, adds regularizer to reg_collection """
        gates = self.get_gates(training, shape=tf.shape(values) if self.local_rep else None)
    ...
    def get_gates(self, is_train, shape=None):
        ...
        with tf.name_scope(self.name):
            if is_train:
                ...
                noise = tf.random.uniform(shape, self.eps, 1.0 - self.eps)
                concrete = tf.nn.sigmoid((tf.math.log(noise) - tf.math.log(1 - noise) + self.log_a) / self.temperature)
            else:
            ...
            if self.hard:
                hard_concrete = tf.cast(tf.greater(clipped_concrete, 0.5), tf.float64)
    ...
    def get_penalty(self, values=None, axis=None):
        ...
        with tf.name_scope(self.name):
            ...
            p_open = tf.nn.sigmoid(self.log_a - self.temperature * tf.math.log(-low / high))
    ...
    def get_sparsity_rate(self, is_train=False):
        ...
        return tf.reduce_mean(tf.cast(is_nonzero, tf.float64))
