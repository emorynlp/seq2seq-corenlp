...
def pick_tensor_for_each_token(h, token_span, average_subwords):
    """ Get pooled representation for each token.

    Args:
        h: Raw representations.
        token_span: List of spans, each span is a list of inclusive indices.
        average_subwords: `True` to average representations.

    Returns:
        Pooled representation, either averaged or picked as the first token according to the ``average_subwords``.
    """
...
def build_optimizer_for_pretrained(model: torch.nn.Module,
                                   pretrained: Optional[torch.nn.Module],
                                   lr=1e-5,
                                   weight_decay=0.01,
                                   eps=1e-8,
                                   transformer_lr=None,
    ...
    pretrained = set(pretrained.parameters()) if pretrained else {}
