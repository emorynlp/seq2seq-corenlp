...
from elit.datasets.parsing.loaders.conll_dataset import CoNLLParsingDataset, append_bos
...
class BiaffineDependencyParser(TorchComponent):
    ...
    def predict(self, data: Any, batch_size=None, batch_max_tokens=None, conll=True, ret_scores=True, **kwargs):
        if not data:
            return []
        ...
        for batch in dataloader:
            ...
            if ret_scores:
                ...
                arcs = arc_scores.softmax(dim=-1)
                rels = rel_scores.softmax(dim=-1)
                if ret_scores is True:
                    arcs = arcs.tolist()
                    rels = rels.tolist()
                else:
                    arcs = arcs.cpu().detach()
                    rels = rels.cpu().detach()
                for arc, rel, l in zip(arcs, rels, batch['sent_length'].tolist()):
                    if ret_scores is True:
                        scores.append({
                            'arc_scores': [x[:l] for x in arc[1:l]],  # Omit the first token as it is ROOT
                            'rel_scores': [[dict(zip(vocab, y)) for y in x[:l]] for x in rel[1:l]],
                        })
                    else:
                        scores.append({
                            'arc_scores': arc[1:l, :l],  # Omit the first token as it is ROOT
                            'rel_scores': rel[1:l, :l, :],
                        })
        outputs = self.post_outputs(predictions, data, order, use_pos, build_data)
        if ret_scores:
            scores = reorder(scores, order)
            outputs = [{'predictions': p, 'scores': s} for p, s in zip(outputs, scores)]
        if flat:
            return outputs[0]
        return outputs
    ...
    def build_dataloader(self,
        ...
        if self.config.transformer:
            prune_max_seq_len = self.config.get('prune_max_seq_len', None)
            if prune_max_seq_len and isinstance(data, str):
                dataset.prune(lambda x: len(x['input_ids']) > prune_max_seq_len, logger)
