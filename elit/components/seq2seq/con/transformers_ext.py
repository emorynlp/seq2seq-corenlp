...
from elit.components.seq2seq.con.constrained_decoding import ShiftReduceProcessor
from elit.components.seq2seq.con.verbalizer import ShiftReduceVerbalizer
from elit.components.seq2seq.dep.constrained_decoding import BracketProcessor
from elit.components.seq2seq.dep.verbalizer import BracketedVerbalizer
...
class ConstrainedDecoding(GenerationMixin):
    def _get_logits_processor(self, repetition_penalty: float, no_repeat_ngram_size: int,
        ...
        if isinstance(verbalizer, ShiftReduceVerbalizer):
            processor = ShiftReduceProcessor(
                self.config.batch,
                ls=self.config.ls,
                sh=self.config.sh,
                rs=self.config.rs,
