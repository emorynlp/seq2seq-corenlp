...
def ner_tsv_to_jsonlines(src, dst=None):
    if not dst:
        dst = os.path.splitext(src)[0] + '.jsonlines'
    part = os.path.splitext(os.path.basename(src))[0]
    with open(dst, 'w') as out:
        for idx, sent in enumerate(read_tsv_as_sents(src)):
            ner = get_entities([x[-1] for x in sent])
            ner = [(x[1], x[2] - 1, x[0]) for x in ner]
            doc = {'doc_key': part, 'sentences': [[x[0] for x in sent]], 'ner': [ner]}
