...
class CharTable:
    ...
    @staticmethod
    def load(path=HANLP_CHAR_TABLE_TXT):
        mapper = {}
        with open(get_resource(path), encoding='utf-8') as src:
            for line in src:
                cells = line.rstrip('\n')
                if len(cells) != 3:
                    continue
...
class JsonCharTable(CharTable):
    ...
    @staticmethod
    def load(path=HANLP_CHAR_TABLE_JSON):
        return load_json(get_resource(path))
