        ...
        try:
            ...
            if installed_version != latest_version:
                ...
                hints = f'[green]Please upgrade to the latest version ({latest_version}) with:[/green]' \
                        f'\n\n\t[yellow]pip install -U elit[/yellow]\n'
...
def get_resource(path: str, save_dir=elit_home(), extract=True, prefix=HANLP_URL, append_location=True,
    ...
    Args:
      path: A local path (which will returned as is) or a remote URL (which will be downloaded, decompressed then
        returned).
      save_dir: Where to store the resource (Default value = :meth:`elit.utils.io_util.hanlp_home`)
      ...
      prefix: A prefix when matched with an URL (path), then that URL is considered to be official. For official
        resources, they will not go to a folder called ``thirdparty`` under :const:`~hanlp_common.constants.ELIT_HOME`.
...
def check_version_conflicts(extras=None):
    ...
    try:
        ...
        error = e.with_context('elit').report()
        return error, extras
    return None, extras
