...
class VocabDict(SerializableDict, Configurable):
    ...
    def _load_vocabs(vd, vocabs: dict, vocab_cls=Vocab):
        ...
        Args:
            vd: The empty VocabDict to fill up.
            vocabs: The config in `dict` type.
    ...
    def config(self):
        config = self.to_dict()
        config['classpath'] = classpath_of(self)
        return config

    @staticmethod
    def from_config(config: dict, **kwargs):
        config.pop('classpath')
        return VocabDict._load_vocabs(VocabDict(), config)
